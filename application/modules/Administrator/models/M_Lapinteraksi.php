...
class M_Lapinteraksi extends CI_Model {
    ...
    function index($nik) {
        $exec = $this->db->select('lap_interaksi.tgl_daftarlap, lap_interaksi.nik, lap_interaksi.nopen, lap_interaksi.pensiunan, lap_interaksi.tgl_lahir_pensiunan, lap_interaksi.penerima, lap_interaksi.tgl_lahir_penerima, lap_interaksi.alamat, lap_interaksi.kelurahan, lap_interaksi.kecamatan, lap_interaksi.kabupaten, lap_interaksi.provinsi, lap_interaksi.kodepos, lap_interaksi.telepon, lap_interaksi.keterangan, lap_interaksi.alamat_valid, lap_interaksi.bertemu_pensiunan, lap_interaksi.hp_status, lap_interaksi.hp_nominal, lap_pict.path')
                ->from('lap_interaksi')
                ->join('lap_pict', 'lap_interaksi.nopen = lap_pict.nopen')
                ->join('norut', 'lap_interaksi.nik = norut.nik')
                ->where('norut.spv', $nik)
                ->get()
                ->result();
        return $exec;
