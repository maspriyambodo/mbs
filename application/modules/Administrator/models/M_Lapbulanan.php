...
class M_Lapbulanan extends CI_Model {
    ...
    function MReportMonthly() {
        ...
        //$exec = $this->db->query('select norut.norut, mst_karyawan.nama_karyawan as uname, mst_karyawan.lokasi_kerja, date_format(mst_karyawan.tanggal_masuk,"%m/%y") as tanggal_masuk, ( select count( lap_interaksi.nom_tb) from lap_interaksi where usr_adm.nik = lap_interaksi.nik and lap_interaksi.syscreatedate between "' . date("y") . '-' . date("m", strtotime("- 1 month ")) . '-25" and "' . date("y") . '-' . date("m") . '-24" AND lap_interaksi.nom_tb > 0) as plaf,( select count(*) from lap_rencana where usr_adm.nik = lap_rencana.nik and syscreatedate between "' . date("y") . '-' . date("m", strtotime("- 1 month ")) . '-25" and "' . date("y") . '-' . date("m") . '-24" ) as ri, ( select count( * ) from lap_interaksi where usr_adm.nik = lap_interaksi.nik and lap_interaksi.syscreatedate between "' . date("y") . '-' . date("m", strtotime("- 1 month ")) . '-25" and "' . date("y") . '-' . date("m") . '-24" ) as hi, ( select count( hp_status ) from lap_interaksi where usr_adm.nik = lap_interaksi.nik and lap_interaksi.hp_status = "y" and lap_interaksi.syscreatedate between "' . date("y") . '-' . date("m", strtotime("- 1 month ")) . '-25" and "' . date("y") . '-' . date("m") . '-24" ) as hp from norut left join usr_adm on norut.nik = usr_adm.nik left join mst_karyawan on norut.nik = mst_karyawan.nik left join lap_interaksi on norut.nik = lap_interaksi.nik left join lap_rencana on norut.nik = lap_rencana.nik group by norut.nik order by norut.norut asc');
