...
function Login() {
    ...
    if (a === '') {
        ...
        $.ajax({
            async: !1,
            type: 'POST',
            url: "https://localhost/Mbs/Login/Proses/",
            data: {
                'uname': a,
                'nik': b
            },
            statusCode: {
                200: function (data) {
                    if (data.hakakses == 1) {
                        window.location.href = "https://localhost/Mbs/Administrator/Dashboard/index"
                    } else if (data.hakakses == 2) {
                        window.location.href = "https://localhost/Mbs/HRIS/Dashboard/index"
                    } else if (data.hakakses == 3) {
                        window.location.href = "https://localhost/Mbs/Sales/Dashboard/index"
                    } else if (data.hakakses == 10) {
                        window.location.href = "https://localhost/Mbs/Sales/Dashboard/index"
                    } else if (data.hakakses == 11) {
                        window.location.href = "https://localhost/Mbs/Telemarketing/Dashboard/index"
                    } else {
                        window.location.href = "https://localhost/Mbs/Logout/index/"
                    }
                },
                201: function (data) {
                    $('#errmsg').show('slow');
